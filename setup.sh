...
#   - 已存在的配置自动跳过，不重复写入
#   - Cron job 采用 upsert 逻辑，自动更新频率等参数
#   - 重复运行安全，任何时候都可以再跑一次
...
# Skill 定义文件位于 mnt/user-data/outputs/fin-intel-skills/
...
echo "  ✓ Heartbeat 已配置（每2分钟，超时300秒，模型 haiku）"

# Cron Jobs（upsert：先删旧任务再添加，确保频率等参数始终更新）
upsert_cron() {
  local name="$1"
  shift
  # 删除旧任务（已不存在时静默跳过）
...
upsert_cron "情绪快照" \
  --cron "0 */4 * * *" \
  --session isolated \
  --message "运行 sentiment skill，读取最近4小时 filtered-news.json，对各板块做情绪分析，推送飞书和 Telegram 情绪日报频道" \
  --model haiku

upsert_cron "板块快报" \
  --cron "0 */3 * * *" \
  --session isolated \
  --message "运行 summarize skill，读取最近3小时 filtered-news.json，按板块生成快报，推送飞书和 Telegram 板块监控频道" \
  --model sonnet
...
upsert_cron "早报" \
  ...
  --tz "Asia/Shanghai" \
  --session isolated \
  --message "运行 summarize skill 早报模式，汇总过去12小时所有内容，生成完整早报摘要推送飞书和 Telegram，同时生成详细内容 PDF 文件发送给用户" \
  --model sonnet
