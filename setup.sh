...
# ════════════════════════════════════════
# 第五步：配置飞书 Webhook + Cron Jobs
# ════════════════════════════════════════
echo ""
echo "[5/5] 配置飞书 Webhook 和定时任务..."

# ── 飞书 Webhook 输入（已配置则跳过）──
get_webhook() {
  local key="$1"
  local label="$2"
  local existing
  ...
  if [ -n "$existing" ] && [ "$existing" != "null" ]; then
    ...
    read -p "  $label Webhook URL: " url
    if [ -n "$url" ]; then
      openclaw config set "$key" "$url"
      echo "  ✓ $label 已配置"
    fi
  fi
}

echo "  （飞书群 → 设置 → 群机器人 → 自定义机器人 获取 URL）"
echo ""
get_webhook "skills.entries.monitor.env.FEISHU_ALERT_WEBHOOK"       "即时预警频道"
get_webhook "skills.entries.sentiment.env.FEISHU_SENTIMENT_WEBHOOK" "情绪日报频道"
get_webhook "skills.entries.summarize.env.FEISHU_SECTOR_WEBHOOK"    "板块监控频道"
get_webhook "skills.entries.summarize.env.FEISHU_MORNING_WEBHOOK"   "早报汇总频道"
